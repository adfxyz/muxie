use crate::paths::{desktop_entry_path, icon_paths, config_path, state_path};
...
fn run_xdg_settings_with_diagnostics(args: &[&str]) {
    match std::process::Command::new("xdg-settings").args(args).output() {
        Ok(output) => {
...
pub fn uninstall(yes: bool, dry_run: bool, restore_default: bool) -> Result<()> {
    ...
    for p in &icons { println!("- Remove icon: {}", p.display()); }
    ...
    if yes {
        ...
        &format!("Delete {} as well? [y/N]", cfg.display()),
        false,
    )? {
        delete_config = true;
    }
    if delete_config { println!("- Remove config: {}", cfg.display()); }
    ...
    for p in &icons {
        if p.exists() {
            if let Err(e) = fs::remove_file(p) { failures.push((p.clone(), e.to_string())); } else { removed.push(p.clone()); }
        }
    }
    // Remove state file (after restore attempt)
    if state.exists() {
        if let Err(e) = remove_state_file() { failures.push((state.clone(), e.to_string())); } else { removed.push(state.clone()); }
    }
    // Remove config if approved
    if delete_config && cfg.exists() {
        if let Err(e) = fs::remove_file(&cfg) { failures.push((cfg.clone(), e.to_string())); } else { removed.push(cfg.clone()); }
    }

    println!("\nSummary:");
    for p in &removed { println!("- Removed: {}", p.display()); }
